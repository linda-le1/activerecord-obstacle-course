...
# ----------------------
  ...
  it '16. returns the names of users who ordered one specific item' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    users = User.select(:name).joins(:items).where("order_items.item_id=#{@item_8.id}").order(:name).distinct.pluck(:name)
    # ------------------------------------------------------------
  ...
  it '18. returns the names of items for a users order' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    items_for_user_3_third_order = Order.joins(:user).where("users.id =#{@user_3.id}")[2].items.pluck(:name)
    # ------------------------------------------------------------
