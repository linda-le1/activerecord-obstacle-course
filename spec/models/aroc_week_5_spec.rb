...
# ----------------------
  ...
  it '26. returns the names of items that are associated with one or more orders' do
    ...
    ordered_items_names = Item.joins(:orders).order(:id).distinct.pluck('items.name')
    ...
    ordered_items_names = Item.order(:id).distinct.joins(:orders).pluck('items.name')
    ordered_items_names = Item.joins(:orders).distinct.order(:id).pluck('items.name')
    # ---------------------------------------------------------------
  ...
  it '27. returns a table of information for all users orders' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    custom_results = User.joins(:orders).select("users.name, count(orders.id) as total_order_count").order("total_order_count").group("users.name")
    # ---------------------------------------------------------------
  ...
  it '28. returns a table of information for all users items' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    custom_results = User.joins(:order_items).select("users.name, count(order_items.id) as total_item_count").group("users.name").order("users.name")
  ...
  it '29. returns a table of information for all users orders and item counts' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    data = User.joins(:order_items).select("users.name as user_name, orders.id as order_id, (orders.amount / count(order_items.id)) as avg_item_cost").group("users.name, orders.id").order("user_name DESC").order("avg_item_cost")
    # ---------------------------------------------------------------
